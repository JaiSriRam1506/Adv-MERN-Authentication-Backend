...
async function userRegistration(userData, req) {
  try {
    ...
    if (user) return { JWT_Token, user };
      ...
      throw new AppError(
        "Unable to Process Registration:" + user,
    ...
    throw new AppError(
      "Unable to Process Registration:" + error,
      StatusCodes.INTERNAL_SERVER_ERROR
    );
  }
}
...
async function userLogin(req) {
  try {
    ...
    if (!unknownAgent) {
      ...
      const token = await Token.findOne({ userId: user._id.toString() });
      if (token) {
    ...
    if (user && validPassword) {
      const jwtToken = await AUTH.createToken({ _id: user?._id.toString() });
...
async function forgotPassword(email) {
  try {
    if (!email) {
      throw new AppError(
        "Please provide Email of the User",
        StatusCodes.BAD_REQUEST
      );
    }
...
async function verifyUser(verificationToken) {
  try {
    ...
    if (!tokenFound) {
      throw new AppError(
        "Token has expired or timeout, Please re-send the verification email",
    ...
    if (user.isVerified) {
      throw new AppError(
        "User Already verified, no need of verification",
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    throw new AppError(
      "Unable to Verify the User: " + error,
      StatusCodes.INTERNAL_SERVER_ERROR
    );
  }
}
...
async function resetPassword(resetToken, password, cPassword) {
  try {
    if (!password || !cPassword || password !== cPassword) {
      throw new AppError(
        "Please Provide Password or Password doesn't match with Confirm Password",
        StatusCodes.BAD_REQUEST
      );
    }
    const hashedToken = hashToken(resetToken);
    ...
    if (!tokenFound) {
      throw new AppError(
        "Token has expired or timeout, Please re-send the Reset email",
        StatusCodes.FORBIDDEN
      );
    }
    ...
    throw new AppError(
      "Unable to Reset the Password: " + error,
      StatusCodes.INTERNAL_SERVER_ERROR
    );
  }
}
...
async function changePassword(req) {
  try {
    ...
    if (!oldPassword || !newPassword || oldPassword === newPassword) {
      throw new AppError(
        "Please Provide Password or New Password shouldn't same with Old Password",
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    throw new AppError(
      "Unable to Change the Password: " + error,
      StatusCodes.INTERNAL_SERVER_ERROR
    );
  }
}
...
async function sendLoginCode(email) {
  try {
    ...
    if (!tokenFound) {
      throw new AppError(
        "Token has expired or timeout, Please re-login to get Login Code",
        StatusCodes.FORBIDDEN
      );
    }
...
async function loginWithCode(req) {
  try {
    ...
    if (!tokenFound) {
      throw new AppError(
        "Token is expired or wrong, Please try-again",
        StatusCodes.NOT_FOUND
      );
    }
    ...
    if (decryptToken !== loginCode) {
      throw new AppError(
        "Token has expired or wrong Token, Please try again",
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    throw new AppError(
      "Something Went Wrong while login with Code: " + error,
      StatusCodes.INTERNAL_SERVER_ERROR
    );
  }
}
...
async function loginWithGoogle(req) {
  try {
    ...
    if (!user) {
      ...
      if (!newUser) {
        throw new AppError(
          "Not able to login/signup using Google Account",
          StatusCodes.INTERNAL_SERVER_ERROR
        );
      }
      if (newUser) {
        const JWT_Token = await AUTH.createToken({
          _id: newUser?._id.toString(),
        });
        return { JWT_Token, user: newUser };
      }
    }
    const jwtToken = await AUTH.createToken({ _id: user?._id.toString() });
    return { jwtToken, user };
    ...
    throw new AppError(
      "Something Went Wrong while login with Google Account: " + error,
      StatusCodes.INTERNAL_SERVER_ERROR
    );
  }
}
