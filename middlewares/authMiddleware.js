...
async function checkAuthentication(req, res, next) {
  ...
  try {
    ...
    if (user.role === "suspended") {
      throw new AppError(
        "User has suspended, Please contact support",
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    if (token && isTokenExists && user) {
      ...
      throw new AppError(
        "Unable to authenticate to the Server,Please try again",
...
async function adminOnly(req, res, next) {
  try {
    if (req.user && req.user.role == "admin") {
      ...
      throw new AppError(
        "User is not authorized to do the required Action",
...
async function isVerified(req, res, next) {
  try {
    if (req.user && req.user.isVerified) next();
      ...
      throw new AppError(
        "User is not verified, Please verify to do the required Action",
        StatusCodes.UNAUTHORIZED
      );
  } catch (error) {
    console.log(error);
